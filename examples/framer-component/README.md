...
A Framer code component that embeds a Worlddots globe with full
...
Worlddots ships as a real Framer building block, not just a screenshot
...
2. Name it **`WorlddotsGlobe`**
3. Replace the editor's contents with
   [`WorlddotsGlobe.tsx`](./WorlddotsGlobe.tsx) from this directory
4. Save (Framer auto-compiles)
5. Drag the `WorlddotsGlobe` chip from your Code section onto any frame
...
`https://worlddots.app/embed?<query-string>`. The query string is built
...
- **`loading="lazy"`** in canvas mode, `loading="eager"` in preview/live
  mode. Keeps the Framer canvas snappy when many WorlddotsGlobe
...
URL directly, or fork Worlddots itself and host your own embed at a
private URL.
...
[Discussion](https://github.com/alevizio/worlddots/discussions) if
