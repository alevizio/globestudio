...
The smallest viable way to put Globestudio on your own page. Single iframe
pointing at a preset URL on `globestudio.app`. No build step, no npm install,
...
- A `prefers-reduced-motion` query that swaps the URL to a static-frame
  ...
  future-proofing — Globestudio already honors the OS-level pref, this is
...
- **Versioned**: you embed `https://globestudio.app/...` and get whatever
  is deployed there at request time. No npm dependency to upgrade.
- **Sandboxed**: nothing from Globestudio can touch your page's DOM, CSS,
  or analytics.
- **Cached**: Vercel + browser caching means subsequent loads are
  near-instant.
- **Mobile-safe**: Globestudio's adaptive DPR + reduced-motion gating run
...
**Notion**: paste the `globestudio.app/looks/...` URL directly — Notion
...
The hosted preset URLs at `globestudio.app/looks/*` are free for commercial use.
